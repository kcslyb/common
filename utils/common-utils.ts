...
export class DateUtils {
   ...
   * @returns {String}
   */
  public static formatStringToDate (formatString: string, format = 'YYYY-MM-DD') {
    if (formatString) {
...
export class CustomUtils {
  ...
  //   const url = window.location.origin + '/file/download' + fileId
  ...
  //     a.href = window.URL.createObjectURL(res.response) // xhr.response is a blob
   ...
   * @return {Function}
   */
  public static debounce (func: () => any, wait = 3000, immediate = false) {
    // 缓存一个定时器id
    let timer: any = null
    let result: any
   ...
   * @returns {Function}
   */
  public static throttle (func: () => {}, wait = 3000) {
